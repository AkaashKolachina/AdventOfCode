...
hand_to_val = {"T":10, "J":0,"Q":12,"K":13,"A":14}

def get_hand_type(hand):
    hand_freqs = Counter(hand)
    num_jokers = hand_freqs[0]
    hand_freqs[0] = 0
    hand_freqs = hand_freqs.values()
    if 5 - num_jokers in hand_freqs:
        return 7 # 5 of a kind
    elif 4 - num_jokers in hand_freqs:
        return 6 # 4 of a kind
    elif 3 - num_jokers in hand_freqs:
        if num_jokers == 1:
    ...
    elif 2 - num_jokers in hand_freqs:
        if Counter(hand_freqs)[2] > 1:
            return 3 # Two Pair
        else:
